...
### 「④ .exp コンバート」を独立モードとして追加 — 検証パイプラインから分離

v4.21.0 では `.exp` を ① 検証モードのルータ枠と ③ 簡易検証モードに組み込み、投入と同時に
自動変換していたが、「変換は検証とは別の機能として独立させたい」という要望により構成を
改めた。Phase 00 のモード選択に 4 つ目のカード **「④ .exp コンバート」** を追加し、
`.exp` → `.txt` / Excel の変換はここだけで行う。

- **④ .exp コンバート**(`src/ui/phases/PhaseExp.tsx`): `.exp` を選ぶだけで復号し、
  FACET用テキスト(.txt)/ 復号テキスト(全変数)/ Excel(.xlsx、11 シート)をダウンロード。
  複数ファイルの一括変換に対応(同名は新しい方で置き換え)。検証は行わない。
  ステッパー非表示の単一画面で、「ホームに戻る」時の確認文言も ④ 向けに用意。
- **投入枠は CLI 可読テキスト専用に戻した**: Phase 03 のルータ枠(`DeviceSlot`)と
  簡易検証モードから `.exp` 自動変換・変換パネル・`accept=.exp` を撤去。
  `.exp` は ④ で `.txt` に変換してから ① / ③ に投入する(案内文をその旨に更新)。
  store の `quickExp` も撤去。
- 復号・変換・XLSX 生成のエンジン(`expDecode` / `expToCli` / `xlsx`)と回帰テストは
  v4.21.0 のまま変更なし。
- ユーザーガイド: 「1.2 四つの利用モード」、新章「9. .exp コンバート」を追加し
  以降の章を繰り下げ。スクリーンショットは ④ の画面で撮り直し。

---

## v4.21.0 — 2026-09-20

### SonicWall `.exp`(Settings Export)の読み込み対応 — 復号 → 変換テキスト → Excel

これまで「難読化バイナリのため意図的に非対応」としていた `.exp` を投入できるようにした。
着手前に形式を裏取りし、`.exp` は暗号化ではなく **`key=value&…` の設定変数リストを
base64 エンコードしたテキスト**(値は URL エンコード、末尾に `&&` が付くことがある)で、
SonicOS 6 系・7 系(Gen7)とも同一方式であることを SonicWall 公式 KB と公開実装
(pryorda/sonicwallRuleParser、faridlav/SonicWallParser)で確認。変数名と列挙値は推測せず
これらの実装と実物サンプルから採用し、**実物の `.exp` で復号結果(507 変数)が同梱の
参照テキストと完全一致**することを確認した。パスワード等の一部の値は機器側で暗号化
されており復号できない(この点はパネル・Excel に明記)。

**使い方**
- Phase 03 のルータ枠、または簡易検証モード(ルータ種別)で `.exp` を選ぶだけ。拡張子が
  `.exp` でなくても中身が base64 の `.exp` なら自動判定する。
- 復号→変換はブラウザ内で完結し、変換後の CLI テキストがそのまま検証に使われる。
- 変換結果パネルから 3 つの成果物をダウンロード:
  1. **FACET 用テキスト(.txt)** — 検証モード/簡易検証モードにそのまま再投入できる CLI 可読テキスト
  2. **復号テキスト** — 全設定変数を 1 行 1 変数で並べたもの(値は percent デコード済み)
  3. **Excel(.xlsx)** — 概要 / インターフェイス / ゾーン / アドレスオブジェクト / アドレスグループ /
     サービスオブジェクト / サービスグループ / アクセスルール / NAT ポリシー / DHCP スコープ /
     全変数 の 11 シート(先頭行固定・列幅自動)

**実装**
- `src/engine/expDecode.ts`: 終端 `&&`・BOM・折り返し改行・パディング不足に対応した復号。
- `src/engine/expToCli.ts`: 変数群 → 構造化モデル → CLI テキスト / Excel シート。変換できない
  項目(アドレス/サービスグループ、FQDN、無効化された NAT、MGMT/U0 等の非 `X*` IF、静的ルート)
  は省略リストとして返し UI に明示する。WAN 側インターフェイスの Ping / HTTPS / SSH 管理が
  有効なら FACET の SEC ヒューリスティックが読む定型行を出力する。
- `src/engine/xlsx.ts`: 依存ライブラリなしの最小 XLSX ライタ(無圧縮 ZIP + SpreadsheetML、
  inlineStr セル、見出しスタイル)。生成物は ZIP/XML 妥当性を機械検証し、実際の Excel 16 で
  修復なしに開けることを確認。ランタイム依存は引き続き react / react-dom のみ。
- パーサ拡張: スペースを含むオブジェクト名の `"…"` 引用(実 SonicOS CLI と同じ)を
  address-object / service-object / access-rule / nat-policy で受け付け、`!` / `#` 行を
  注釈として認識済み扱いに(変換テキストの読み戻しが未対応行ゼロになる)。
- 回帰テスト 29 件追加(復号・抽出・CLI 往復・verify/pathTrace・パーサ拡張・XLSX 構造・
  ソース衛生)、計 224 ケース。
- ダブルチェックで見つけて直したもの: (1) XLSX ライタの XML 制御文字フィルタが生の制御文字で
  ソースに書き込まれており、dev サーバとテストでは動くのに単一 HTML の本番ビルドでは
  ブラウザが正規表現エラーで起動しなかった → エスケープ表記に修正し、ソースに生の制御文字が
  混入していないことを検証する回帰テストを追加。(2) `.exp` 投入後に「サンプルコンフィグを
  読み込む」を押すと、サンプルに差し替わったスロットの下に古い `.exp` の変換パネルが
  残っていた → スロットの現在の設定が変換テキストと一致する間だけパネルを表示する。

---

## v4.20.2 — 2026-09-19

### 本番展開前の総点検 — 経路トレース表記の修正と文書の是正

本番展開前に、ビルド/テスト/型・CLAUDE.md の不変条件(エンジン DOM-free、UI→engine の
import 境界、localStorage 不使用、外部 CDN なし、サンプル匿名化、deprecated ファイル
未変更)・バージョン整合・本番配信バージョン・成果物テキスト・文書鮮度を横断点検。
不変条件はすべて良好。以下を修正。

- **経路トレースの表記欠陥**: タグなし(untagged)サブネットを送信元/宛先にすると、
  ホップの説明が「R1 の **VLAN-** 内ホスト」のように無意味な `VLAN-` になっていた
  (`(vlan || '-')` による)。「タグなしセグメント」と表記するよう SRC/L2/RT/DST の
  4箇所を修正し、回帰テスト2件を追加(計195ケース)。
- **公開手順書の実態不一致**: `docs/PUBLISHING.md` と `CLAUDE.md` は「Cloudflare Pages /
  pages.dev」を案内していたが、実際の本番は Cloudflare Workers の静的アセット配信
  (`wrangler.jsonc`、`main` への push で自動デプロイ、URL は workers.dev)。手順書どおりに
  作業すると現行とは別の配信を新設してしまうため、現行方式を正として書き直し、Pages は
  代替手順として残した。
- **文書の陳腐化した数値**: README / CLAUDE.md / ARCHITECTURE / PARSER-NOTES / PUBLISHING
  に残っていた「テスト 46 ケース」「配布物 〜220KB」を現状(195 ケース・〜330KB)に更新。
  公開前チェックリストに③簡易検証モードの動作確認を追加し、README のクイックスタートに
  簡易検証モードを記載。ユーザーガイド生成の「新規：CAP」表記(Sprint 2 から稼働済み)も
  「補足：」に改め、`.docx` を再生成。
...
### 全モードE2E検証で見つかった成果物の欠陥を修正

3モード(検証/作成/簡易)を Phase 0 から成果物出力まで実操作し、各成果物を
エンジン直呼びのグラウンドトゥルースと突合。3モードとも成果物(findings・
到達性マトリクス・経路トレース・JSON/Markdown出力・生成config・簡易結果)は
GTと完全一致で、検証ロジックとUI/エクスポートの整合性は良好だった。その過程で
見つかった成果物レベルの欠陥を修正。

- **検証レポートの注記の誤字**: 「設定上の不整合・**脳閃**・リスク」という無意味な
  語を「見落とし」に訂正。
- **作成モードの生成configに実機投入不可の行**: Cisco生成物の
  `enable secret 9 $facet$generated$` は有効な type-9(scrypt)ハッシュではなく
  実機IOSが拒否する。作成モードは「そのまま実機投入可」と謳うため、実機が受理でき
  かつ「要変更」が値から明白な type-0(平文入力)形式
  `enable secret 0 FACET-CHANGE-ME-BEFORE-DEPLOY` +警告コメントに変更。
  パーサは `^enable\s+secret\b` で enableSecret を読み戻すため往復保証は維持。
  回帰テスト1件追加(計193ケース)。
- **結果ヒーローの二重カウント表示の紛らわしさ**: 「指摘件数(エラー/不足)」と
  「物理ポート単位の状態集計」が同じ語ラベルで隣接し矛盾して見えた。verdictを
  「指摘 エラー…」に、ポート集計に「ポート状態の内訳(物理ポート単位・指摘件数とは
  別集計)」ラベルと「構成ポート/確認ポート/不足ポート/エラーポート」キャプションを付与。
- **L3ゲートウェイ欠如の指摘文言を機種非依存化**: 修正提案が未アップロードの
  「SonicWall」を指しており簡易検証モードのスコープと不整合だったため、
  「ルータのサブインターフェイス、またはL3スイッチのSVI」という機種非依存の表現に変更。
