...
### .exp コンバート追加後の総合監査 — 見つかった不備の修正

v4.21.0 / v4.22.0 で追加した `.exp` 関連機能とその周辺(パーサ・store・Header・文書)を
通しで監査し、以下を修正した。

**エンジン**
- **Excel 生成が実機規模の `.exp` で失敗する(致命的)**: ZIP を `push(...bytes)` /
  `[...a, ...b]` の配列展開で連結していたため、シート XML が数十万バイトを超えると
  JavaScript の引数上限で `RangeError` になり、Excel を出力できなかった。テストの小さな
  合成データでは顕在化しない。`Uint8Array` の `set` による連結に変更し、20,000 行(約 3MB)の
  シートで EOCD のオフセット整合まで確認する回帰テストを追加。
- **セル上限超過**: 1 セル 32,767 文字を超える値(証明書・長い一覧など)があると Excel が
  「修復」を要求する。切り詰めて `…[切詰]` を付けるようにした。空列があると `<cols>` の
  幅が `NaN` になる潜在不具合も修正。
- **パーサ: 引用付きゾーン名**: v4.21.0 で address-object / access-rule には `"…"` 引用を
  入れたが、`interface` ブロックの `zone` 行は未対応で、`.exp` 変換が出力する
  `zone "Trusted Zone"` を `"Trusted` と誤読していた。修正し、`.exp` → CLI → パーサの
  往復テストを追加。
- **復号済みテキストの受け付け**: これまで base64 でない入力は一律エラーにしていたが、
  エラー文言で「復号済みのテキストかもしれない」と示唆しながら受け付けていなかった。
  `key=value` の並び(1 行 1 変数、または `&` 区切り)なら復号済みとしてそのまま読む。
  値に `&` を含み得るため、複数行なら改行のみで区切る。

**UI**
- **「ホームに戻る」の判定不整合**: Header は ④ に入った時点で常に確認ダイアログを出す一方、
  画面内の「← ホームに戻る」は変換結果があっても確認なしで破棄していた。変換結果を store
  (`expResults`)に移し、両者が「結果が残っていれば確認、無ければ即座に戻る」を共有する
  ように統一(v4.20.0 の Medium-14/15 と同じ方針)。
- 使われなくなっていた `isExpUpload` を削除。

**文書**
- `docs/ARCHITECTURE.md` の PhaseId 一覧に `exp` を追加し、④ の状態の持ち方を記載。
- ユーザーガイドの版数を更新し再生成。
