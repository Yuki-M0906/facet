...
ウィザード遷移は `phase: PhaseId`(`mode|select|topo|upload|analyze|results|complete`)。
`PhaseRouter` が現在の phase 名に応じて該当コンポーネントを描画する。
...
カバー範囲:
- expandIfRange(2)
- parseCisco(8)、parseSonicWall(7)
- evalFW(4)+ svcMatch 双方向(6)
- full verify(7)+ matrix(2)
- pathTrace(4)+ same-subnet(4)
- shadowed / permissive(2)

新ルール追加 / バグ修正のたびにケース追加。`npm test` が緑のままを維持する。
