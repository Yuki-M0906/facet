...
### SonicWall `.exp` 対応(2026-09-20、v4.21.0)
- [x] `.exp` は base64+URL エンコードの `key=value&…` であることを公式 KB・公開実装・実物
      サンプルで裏取りし、ブラウザ内で復号 → FACET 可読 CLI テキストへ変換して投入できる
      ようにした。復号テキスト / FACET 用テキスト / Excel(11 シート、依存ライブラリなしの
      自前 XLSX ライタ)をダウンロード可。省略項目(グループ、FQDN、無効 NAT、静的ルート等)は
      パネルと Excel に明示。詳細は `docs/PARSER-NOTES.md` の「.exp」節
