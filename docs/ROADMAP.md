...
- [x] Phase 間の遷移アニメーション・ローディング状態の統一 — **v4.20.0**(2026-07-11)
      `.phase`(全フェーズ共通の入場アニメーション)・`.analyzing .shim`・
      `.complete .gem` を `prefers-reduced-motion: reduce` 時に一括で無効化。
      唯一の実運用中ローディング(PhaseAnalyze の演出2.5秒)も同設定時は大幅短縮。
      唯一の未処理だった非同期処理(PhaseQuick のファイル読込)に
      `.builder-generate-status.pending` を再利用した軽量インジケータを追加。
      フルスクリーンの「検証中…」演出は verify→results の1箇所限定という方針を
      明文化(他の非同期処理は今後もインラインバッジで対応する)
...
- [x] 残り分(Low/些末/一部保留Medium)を並行監査で再洗い出し・独立検証した上で
      全件対応 — **v4.20.0**(2026-07-11)。詳細は CHANGELOG.md 参照。パーサ・
      検証エンジン・作成モードUI・トポロジーUI・アクセシビリティ・ドキュメント・
      テストカバレッジの各領域にまたがる約44件の指摘(うち3件は再調査で
      Medium相当と判明し格上げ対応)。CHANGELOG.md 未収載の詳細根拠は
      本セッションのワークフロー実行記録を参照(個票の永続化はしない方針)

### SonicWall `.exp` 対応(2026-09-20、v4.21.0 → v4.22.0 で独立モード化)
- [x] `.exp` は base64+URL エンコードの `key=value&…` であることを公式 KB・公開実装・実物
      サンプルで裏取りし、ブラウザ内で復号 → FACET 可読 CLI テキストへ変換。復号テキスト /
      FACET 用テキスト / Excel(11 シート、依存ライブラリなしの自前 XLSX ライタ)をダウンロード可。
      省略項目(グループ、FQDN、無効 NAT、静的ルート等)はパネルと Excel に明示。
      詳細は `docs/PARSER-NOTES.md` の「.exp」節
- [x] v4.22.0: ユーザ判断により「④ .exp コンバート」を Phase 00 の独立モードにし、
      投入枠(① / ③)からは `.exp` 自動変換を撤去(変換と検証を混ぜない)
- [ ] 静的ルート・機器名(hostname)の `.exp` 側キー名は未確認のため未変換。実機の `.exp`
      で該当変数を確認できたら追加する(候補: `firewallName`)
