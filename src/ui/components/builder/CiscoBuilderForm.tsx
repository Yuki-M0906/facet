...
import type { CiscoBuilderDraft, Device, StpVariant } from '@engine/types';
...
export function CiscoBuilderForm({ device, draft, onChange }: Props) {
  const caps = (device.model as { capabilities?: { stpVariants?: readonly StpVariant[] } }).capabilities;
  ...
  return (
    <div>
      ...
      <div className="builder-section">
        ...
        {draft.vlans.map((v, i) => (
          <div className="builder-row" key={i}>
            ...
            <input type="text" value={v.id} placeholder="10" onChange={(e) => updateVlan(i, { id: e.target.value })} style={{ maxWidth: 70 }} />
            <span className="lbl">名前</span>
            <input type="text" value={v.name} placeholder="STAFF" onChange={(e) => updateVlan(i, { name: e.target.value })} />
      ...
      <div className="builder-section">
        <div className="builder-section-title">ポート設定({draft.ports.length} ポート)</div>
        <div className="builder-scroll">
          {draft.ports.map((p, i) => (
            <div className="builder-row" key={p.iface}>
      ...
      <div className="builder-section">
        ...
        {draft.svis.map((s, i) => (
          <div className="builder-row" key={i}>
            ...
            <input type="text" value={s.ip} placeholder="192.168.10.1" onChange={(e) => updateSvi(i, { ip: e.target.value })} />
            <span className="lbl">Mask</span>
            <input type="text" value={s.mask} placeholder="255.255.255.0" onChange={(e) => updateSvi(i, { mask: e.target.value })} />
