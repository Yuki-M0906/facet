...
body.push(H2("1.2 四つの利用モード"));
body.push(P("FACETには、目的に応じた4つの入り口があります。①検証モードと②作成モードは、その先の検証ロジックとレポート画面が完全に共通です。③簡易検証モードは、1台だけを手早く確認するための独立した近道です。④ .exp コンバートは検証を行わず、SonicWall の Settings Export（.exp）をテキストと Excel に変換するだけの独立した道具です。"));
body.push(shot("00_mode.png"));
body.push(caption("Phase00の画面：検証モード / 作成モード / 簡易検証モード / .exp コンバートの選択"));
body.push(tbl([2400,8066],["モード","向いているケース"],[
 ...
 ["④ .exp コンバート",[P([run("SonicWall の",{}),run("Settings Export（.exp）しか手元にない",{bold:true}),run("場合。.exp をブラウザ内で復号し、FACET に投入できる CLI テキスト（.txt）と、人が読み合わせるための Excel（.xlsx）に変換します。検証は行いません。変換した .txt を①や③に投入して検証します。詳しくは「9. .exp コンバート」参照。")],{after:0,size:20})]]
]));
body.push(spacer(40));
...
body.push(P("検証はフェーズを順に進めます。画面上部のステップバーで現在位置が分かります。「検証モード」と「作成モード」は Phase 03 の中身だけが切り替わり、それ以外は完全に共通です。なお「簡易検証モード」はこのフェーズ列を通らず、機種選択とアップロードだけで結果画面へ直行する近道です（詳細は「8. 簡易検証モード」）。「.exp コンバート」も同様にフェーズ列を通らない単独画面で、検証は行わず変換だけを行います（詳細は「9. .exp コンバート」）。"));
...
body.push(tbl([2400,8066],["機器","投入する内容"],[
 ["SonicWall（ルータ）",[P([run("CLIの可読テキスト",{bold:true}),run("を読みます。Settings Export（.exp）しかない場合は「9. .exp コンバート」で .txt に変換してから投入してください。アドレスオブジェクト・サービスオブジェクト・アクセスルール・NATを含めるとFW検証が有効になります。")],{after:0,size:20})]],
...
body.push(step("「ファイル選択」からその機器のコンフィグテキストを1つアップロードすると、即座に結果画面へ進みます（SonicWall の .exp しかない場合は「9. .exp コンバート」で .txt に変換してから）。","sq"));
...
/* ===== 9. .exp コンバート ===== */
body.push(H1("9. .exp コンバート（SonicWall Settings Export の変換）"));
body.push(shot("13_exp_convert.png"));
body.push(caption(".exp コンバート：.exp を選ぶだけで復号・変換され、3 種類の成果物をダウンロードできる"));
body.push(P("SonicWall の管理画面から書き出した Settings Export（.exp）しか手元にないときは、Phase00 で「④ .exp コンバート」を選びます。.exp は「設定変数の一覧を base64 で符号化したテキスト」なので、FACET がブラウザ内で復号し、検証に投入できる CLI 可読テキストと、人が読み合わせるための Excel に変換します。このモードは変換専用で、検証は行いません。"));
body.push(H2("9.1 手順"));
body.push(step("Phase00で「④ .exp コンバート」を選びます。","sq"));
body.push(step("「ファイル選択」で .exp を選びます。複数ファイルをまとめて選ぶこともできます。選んだ直後にブラウザ内で復号・変換され、ファイルごとに結果パネルが表示されます。","sq"));
...
body.push(spacer(40));
body.push(callout("注意：変換で省略される項目があります",
 ["アドレスグループ・サービスグループ（FACET はグループ展開に未対応）、FQDN オブジェクト、無効化された NAT ポリシー、MGMT などの X 以外のインターフェイス、静的ルートは CLI テキストには含めません。省略した項目はパネルの「省略・注記した項目」と Excel に明示されます。パスワード等の暗号化された値は復号できません。ファイルは外部に送信されません。"],
 "FBF3DD",GOLD));

/* ===== 10. 結果の読み方 ===== */
body.push(H1("10. 検証レポートの読み方"));
...
body.push(bullet("SonicWallの検証はCLIの可読テキスト前提です。Settings Export（.exp）は「④ .exp コンバート」で変換できますが、グループ・FQDN・静的ルートなど一部の項目は変換対象外です（パネルに明示）。SonicOSのバージョンや記法によっては読み取れない記述があります。"));
