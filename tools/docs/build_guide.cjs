...
body.push(new Paragraph({spacing:{before:900,after:0},alignment:AlignmentType.CENTER,
  children:[new TextRun({text:"版数：v4.16.1 ・ 発行日：2026年7月6日",size:20,color:"777777"})]}));
...
body.push(H2("1.2 二つの利用モード"));
body.push(P("FACETには、目的に応じた2つの入り口があります。どちらを選んでも、その先の検証ロジックとレポート画面は完全に共通です。"));
body.push(shot("00_mode.png"));
body.push(caption("Phase00の画面：検証モード / 作成モードの選択"));
body.push(tbl([2400,8066],["モード","向いているケース"],[
 ["① 検証モード",[P([run("手元に",{}),run("既存のコンフィグファイル",{bold:true}),run("がある場合。running-config や SonicOS CLI のテキストをそのまま投入して検証します。")],{after:0,size:20})]],
...
body.push(P("検証はフェーズを順に進めます。画面上部のステップバーで現在位置が分かります。「検証モード」と「作成モード」は Phase 03 の中身だけが切り替わり、それ以外は完全に共通です。"));
...
/* ===== 11. バージョン履歴 ===== */
body.push(H1("11. バージョン履歴の確認"));
body.push(P([run("画面右上のバージョンバッジ（例："),mono("v4.16.1"),run("）をクリックすると、これまでの変更履歴がモーダルで表示されます。どのバージョンで何が変わったかをこの画面だけで確認できます。")]));

/* ===== 12. 注意事項 ===== */
body.push(H1("12. ご利用上の注意"));
...
body.push(bullet("作成モードで生成したコンフィグも、あくまで「案」です。検証結果が良好でも、実機への投入前には必ず内容をレビューしてください。"));
