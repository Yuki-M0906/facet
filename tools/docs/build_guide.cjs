...
body.push(tbl([2400,8066],["機器","投入する内容"],[
 ["SonicWall（ルータ）",[P([run("CLIの可読テキスト",{bold:true}),run("、または"),run("Settings Export（.exp）",{bold:true}),run("をそのまま投入できます。.exp はブラウザ内で復号され、FACET用テキスト・復号テキスト・Excelをダウンロードできます（6.3参照）。アドレスオブジェクト・サービスオブジェクト・アクセスルール・NATを含めるとFW検証が有効になります。")],{after:0,size:20})]],
...
body.push(H2("6.3 SonicWall の Settings Export（.exp）を投入する"));
body.push(shot("13_exp_convert.png"));
body.push(caption(".exp を投入すると復号・変換され、3 種類の成果物をダウンロードできる"));
body.push(P("SonicWall の管理画面から書き出した Settings Export（.exp）は、ルータの枠にそのまま選ぶだけで使えます。.exp は「設定変数の一覧を base64 で符号化したテキスト」なので、FACET がブラウザ内で復号し、検証に使える形（CLI 可読テキスト）へ変換します。変換後は枠の直下に結果パネルが表示されます。"));
body.push(tbl([3000,7466],["ダウンロード","内容"],[
 ["FACET用テキスト（.txt）","変換後の CLI 可読テキスト。検証モード・簡易検証モードにそのまま再投入できます。"],
...
body.push(callout("注意：変換で省略される項目があります",
 ["アドレスグループ・サービスグループ（FACET はグループ展開に未対応）、FQDN オブジェクト、無効化された NAT ポリシー、MGMT などの X 以外のインターフェイス、静的ルートは CLI テキストには含めません。省略した項目はパネルの「省略・注記した項目」と Excel に明示されます。パスワード等の暗号化された値は復号できません。"],
 "FBF3DD",GOLD));
...
body.push(step("「ファイル選択」からその機器のコンフィグテキストを1つアップロードすると、即座に結果画面へ進みます。ルータの場合は Settings Export（.exp）も選べ、復号・変換された成果物（FACET用テキスト・復号テキスト・Excel）を結果画面からダウンロードできます（6.3参照）。","sq"));
...
body.push(bullet("SonicWallはCLIの可読テキスト、またはSettings Export（.exp）を投入します。.exp は復号して変換しますが、グループ・FQDN・静的ルートなど一部の項目は変換対象外です（パネルに明示）。SonicOSのバージョンや記法によっては読み取れない記述があります。"));
