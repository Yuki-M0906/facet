...
async function main() {
  ...
  /* ===== ④ .exp コンバート(v4.22.0 で独立モード化)===== */
  {
    server = await startPreviewServer();
    const page = await browser.newPage({ viewport: { width: 1280, height: 900 }, deviceScaleFactor: 2 });
    console.log('.exp コンバートを撮影中...');

    await page.goto(BASE, { waitUntil: 'networkidle' });
    await page.getByRole('button', { name: /このモードで進む/ }).nth(3).click();
    await page.waitForSelector('text=.exp コンバート');

    /* 匿名の .exp(base64 の key=value 群、ACME / RFC1918 / TEST-NET のみ)を投入し、
     * 変換結果パネルを撮る。 */
